...
def inference_demo(hp):
    ...
    try:
        ...
        # Get ground truth data from the first sample for comparison
        try:
            gt_sample = next(iter(dataset))
            gt_coords, gt_fields_chunk = gt_sample
            gt_fields = gt_fields_chunk.unsqueeze(0).to(device)  # [1, T, N, C]
            print(f"Loaded ground truth sample with shape: {gt_fields.shape}")
        except Exception as e:
            gt_fields = None
            print(f"Warning: Could not load ground truth sample: {e}")
    except Exception as e:
    ...
    if ENCODER_TYPE == "HyperNetwork_Perceiver_v22":
        ...
        encoder = HyperNetwork_GINO(
            ...
            latent_grid_size=getattr(hp, "latent_grid_size", 64)
        ).to(device)
    else:
    ...
    with torch.no_grad():
        ...
        vis_channel = getattr(hp, "vis_channel", 3)
        ...
        if has_gt:
            gt_field = field_normalizer.denormalize(gt_fields).detach().cpu().numpy()
            data0_pred = field[0, 0]
            data0_gt = gt_field[0, 0]
            
            if data0_pred.ndim == 2 and data0_pred.shape[1] > 1:
                values0_pred = data0_pred[:, vis_channel] if vis_channel < data0_pred.shape[1] else np.linalg.norm(data0_pred, axis=1)
                values0_gt = data0_gt[:, vis_channel] if vis_channel < data0_gt.shape[1] else np.linalg.norm(data0_gt, axis=1)
            else:
                values0_pred = data0_pred.squeeze()
                values0_gt = data0_gt.squeeze()
            
            # Create side-by-side comparison plot
            fig, (ax1, ax2) = plt.subplots(1, 2, figsize=(16, 6))
            
            tpc1 = ax1.tripcolor(tri, values0_pred, shading="gouraud", cmap="viridis")
            ax1.set_aspect('equal')
            ax1.set_xlabel("x")
            ax1.set_ylabel("y")
            title1 = ax1.set_title("Predicted Field (time=0)")
            cbar1 = plt.colorbar(tpc1, ax=ax1, label="Velocity Magnitude")
            
            tpc2 = ax2.tripcolor(tri, values0_gt, shading="gouraud", cmap="viridis")
            ax2.set_aspect('equal')
            ax2.set_xlabel("x")
            ax2.set_ylabel("y")
            title2 = ax2.set_title("Ground Truth Field (time=0)")
            cbar2 = plt.colorbar(tpc2, ax=ax2, label="Velocity Magnitude")
            
            # Sync colorbar ranges
            vmin = min(values0_pred.min(), values0_gt.min())
            vmax = max(values0_pred.max(), values0_gt.max())
            tpc1.set_clim(vmin, vmax)
            tpc2.set_clim(vmin, vmax)
            
            def update(t):
                data_pred = field[0, t]
                data_gt = gt_field[0, t]
                if data_pred.ndim == 2 and data_pred.shape[1] > 1:
                    values_pred = data_pred[:, vis_channel] if vis_channel < data_pred.shape[1] else np.linalg.norm(data_pred, axis=1)
                    values_gt = data_gt[:, vis_channel] if vis_channel < data_gt.shape[1] else np.linalg.norm(data_gt, axis=1)
                else:
                    values_pred = data_pred.squeeze()
                    values_gt = data_gt.squeeze()
                
                tpc1.set_array(values_pred)
                tpc2.set_array(values_gt)
                title1.set_text(f"Predicted Field (time={t})")
                title2.set_text(f"Ground Truth Field (time={t})")
                return tpc1, tpc2, title1, title2
            
            ani = animation.FuncAnimation(fig, update, frames=range(frames), interval=80, blit=False)
        else:
            # Single plot without ground truth comparison
        ...
        base_name = "generated_trajectory_with_gt" if has_gt else "generated_trajectory"
        ext = ".gif"
        counter = 0
