...
def train(hp):
    ...
    COORD_DIM = norm_cfg.get("coord_dim", 0) if norm_cfg else 0
    FIELD_DIM = norm_cfg.get("field_dim", 0) if norm_cfg else 0
    ...
    if ENCODER_TYPE == "HyperNetwork_Perceiver_v22":
        ...
        print("Using Perceiver_v24-based HyperNetwork with spherical coordinate support")
        encoder = HyperNetwork_Perceiver_v24(
            ...
            num_tokens=NUM_TOKENS,
            use_node_type=getattr(hp, "use_node_type", False),
            is_spherical=getattr(hp, "is_spherical", False)
        ).to(device)
    elif ENCODER_TYPE == "HyperNetwork_GINO":
        print("Using HyperNetwork_GINO for autoencoder-style reconstruction")
    ...
    if RENDERER_TYPE == "GaborRenderer_v22":
        ...
        print("Using GaborRenderer_GINO for autoencoder-style reconstruction")
    ...
    for epoch in epoch_pbar:
        ...
        for step, (coords_batch, traj_batch) in enumerate(dataloader):
            ...
            # ====================================================
                ...
                d_val = getattr(hp, "teacher_d", 1500.0)
