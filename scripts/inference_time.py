...
from src.models import HyperNetwork, HyperNetwork_FA, HyperNetwork_AP, HyperNetwork_ST, HyperNetwork_Perceiver, HyperNetwork_MultiLatentPerceiver, CNFRenderer, GaborRenderer, SpatialTemporalRenderer
...
def inference_demo(hp):
    ...
    elif ENCODER_TYPE == "HyperNetwork_MultiLatentPerceiver":
        print("Using MultiLatentPerceiver-based HyperNetwork")
        from src.models import HyperNetwork_MultiLatentPerceiver
        encoder = HyperNetwork_MultiLatentPerceiver(
            t_chunk=T_CHUNK,
            channel_in=C_OUT,
            latent_dim=LATENT_DIM,
            hidden_dim=HIDDEN_DIM,
    ...
    if RENDERER_TYPE == "SpatialTemporalRenderer":
        print("Using SpatialTemporalRenderer")
        cnf = SpatialTemporalRenderer(num_tokens=NUM_TOKENS, latent_dim=LATENT_DIM, coord_dim=2, time_dim=1, channel_out=C_OUT, hidden_dim=HIDDEN_DIM, num_layers=NUM_LAYERS_CNF).to(device)
    elif RENDERER_TYPE == "GaborRenderer":
    ...
    with torch.no_grad():
        ...
        if RENDERER_TYPE == "SpatialTemporalRenderer":
            time_steps = torch.linspace(0, 1, T_CHUNK, device=device)
            trajectory_pred_norm = cnf(z1_gen, coords_norm, time_steps)
        else:
