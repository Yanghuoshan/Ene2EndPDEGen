...
def inference_demo(hp):
    ...
    if ENCODER_TYPE == "HyperNetwork_Perceiver_v22":
        print("Using FA-based HyperNetwork_Perceiver_v22")
    ...
    if RENDERER_TYPE == "GaborRenderer_v22":
        print("Using MFN-based GaborRenderer_v22")
        cnf = GaborRenderer_v22(latent_dim=LATENT_DIM, coord_dim=2, t_chunk=T_CHUNK, channel_out=C_OUT, hidden_dim=HIDDEN_DIM, num_layers=NUM_LAYERS_CNF).to(device)
    else:
    ...
    with torch.no_grad():
        ...
        # 2. We want completely noise-driven generation, so we set t_target to a fixed value (e.g., all ones) to indicate "generate" mode
        ...
        if field.shape[-1] > 1:
            all_values = field[0, ..., 0]  # Only visualize vorticity (channel 0)
        else:
            all_values = field[0].squeeze(-1)
        ...
        if data.ndim == 2 and data.shape[1] > 1:
            values = data[:, 0]  # Only visualize vorticity (channel 0)
        else:
            values = data.squeeze()
        values_2d = values.reshape(N_phi, N_theta)
        
        im = ax.imshow(values_2d.T, cmap='RdBu_r', origin='lower', aspect='auto', extent=[0, 360, -90, 90], vmin=vmin, vmax=vmax)
