...
def train(hp):
    ...
    try:
        # Build subset selection from config.
        all_h5_files = sorted(glob(os.path.join(DATASET_PATH, "*.h5")))
        total_len = len(all_h5_files)
        if total_len == 0:
            raise ValueError(f"No h5 files found in {DATASET_PATH}")

        if SIM_INDICES_CFG is not None:
            if isinstance(SIM_INDICES_CFG, str):
                sim_indices = [int(x.strip()) for x in SIM_INDICES_CFG.split(",") if x.strip()]
            elif isinstance(SIM_INDICES_CFG, (list, tuple)):
                sim_indices = [int(x) for x in SIM_INDICES_CFG]
            else:
                sim_indices = [int(SIM_INDICES_CFG)]
            sim_indices = list(dict.fromkeys(sim_indices))
            invalid = [i for i in sim_indices if i < 0 or i >= total_len]
            if invalid:
                raise ValueError(
                    f"sim_indices contains out-of-range values: {invalid}. "
                    f"Valid range is [0, {total_len - 1}]"
                )
        else:
            if DATA_RATIO <= 0:
                raise ValueError(f"data_ratio must be > 0, got {DATA_RATIO}")
            data_ratio = min(DATA_RATIO, 1.0)
            target_count = max(1, int(total_len * data_ratio))
            if MAX_SIMS is not None:
                max_sims = int(MAX_SIMS)
                if max_sims <= 0:
                    raise ValueError(f"max_sims must be > 0, got {MAX_SIMS}")
                target_count = min(target_count, max_sims)

            rng = np.random.default_rng(SUBSET_SEED)
            if target_count < total_len:
                sim_indices = rng.choice(total_len, size=target_count, replace=False).tolist()
                sim_indices.sort()
            else:
                sim_indices = list(range(total_len))

        print(
            f"Using shallow subset: {len(sim_indices)}/{total_len} trajectories "
            f"(data_ratio={DATA_RATIO}, max_sims={MAX_SIMS}, subset_seed={SUBSET_SEED})"
        )
    ...
    for epoch in epoch_pbar:
        ...
        for step, (coords_batch, traj_batch) in enumerate(dataloader):
            ...
            # ====================================================
                ...
                d_val = getattr(hp, "teacher_d", 3000.0)
