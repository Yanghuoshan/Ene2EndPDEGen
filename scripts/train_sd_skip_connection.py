    ...
    for epoch in epoch_pbar:
        ...
        for step, (coords_batch, traj_batch) in enumerate(dataloader):
            ...
            if epoch < PHASE1_EPOCHS:
                t = torch.rand(B, device=device) * T_EPS1
                ...
                recon_loss = (l_recon_unweighted.reshape(B, -1).mean(dim=1)).mean()
                ...
                l_diff_unweighted = torch.zeros_like(l_recon_unweighted)
            ...
            # ====================================================
                ...
                ratio = 1.0 - (1.0 / (q_val ** (global_step / d_val))) * n_t
                ...
                # Apply rigorous skip connection integration
                x_pred = c_skip * x_noisy_query + c_out * f_theta
                ...
                with torch.no_grad():
                    ...
                    c_skip_teach = (sigma_data**2) / (t_teacher_expand**2 + sigma_data**2)
                    c_out_teach = (t_teacher_expand * sigma_data) / torch.sqrt(t_teacher_expand**2 + sigma_data**2)
                    x_pred_teacher = c_skip_teach * x_noisy_teacher + c_out_teach * f_theta_teacher

                # ===== C. Data-Space Supervision =====
                loss_weight = (t**2 + sigma_data**2) / (t * sigma_data + 1e-8)**2
                
                small_noise_mask1 = (t < T_EPS1).float().view(B, 1, 1, 1)
                small_noise_mask2 = (t < T_EPS2).float().view(B, 1, 1, 1)

                # L_recon: At small noise, force f_theta to learn gt
                l_recon_unweighted = compute_element_loss(f_theta, x_real) * small_noise_mask1
                recon_loss = (l_recon_unweighted.reshape(B, -1).mean(dim=1)).sum() / (small_noise_mask1.sum() + 1e-8)

                # L_diff: At small noise, use gt to guide x_pred
                l_diff_unweighted = compute_element_loss(x_pred, x_real) * small_noise_mask2
                diff_loss = (l_diff_unweighted.reshape(B, -1).mean(dim=1) * loss_weight).sum() / (small_noise_mask2.sum() + 1e-8)
                ...
                gene_loss = (l_gene_unweighted.reshape(B, -1).mean(dim=1) * loss_weight).mean()

                # Stage 2: Train generation with full noise range and other losses
                loss = LAMBDA_GENE * gene_loss + LAMBDA_RECON * recon_loss + LAMBDA_DIFF * diff_loss
            ...
            diff_b = l_diff_unweighted.reshape(B, -1).mean(dim=1)
            gene_b = l_gene_unweighted.reshape(B, -1).mean(dim=1)

            if epoch < PHASE1_EPOCHS:
                sample_loss_val_reduced = recon_b.detach()
            else:
                sample_loss_val_reduced = (LAMBDA_GENE * gene_b * loss_weight + 
                                           LAMBDA_RECON * recon_b + 
                                           LAMBDA_DIFF * diff_b * loss_weight).detach()
            for i in range(B):
                ti = t[i].item()
        ...
        avg_diff_loss = epoch_diff_loss / (step + 1)
        ...
        writer.add_scalar('Loss/diff_epoch', avg_diff_loss, epoch + 1)
        ...
        epoch_pbar.set_postfix({
            ...
            'diff': f"{avg_diff_loss:.4f}",
            'gene': f"{avg_gene_loss:.4f}",
            'g_enc': f"{avg_gnorm_enc:.2f}",
            'g_cnf': f"{avg_gnorm_cnf:.2f}"
        })
        print(
            f"\nEpoch {epoch+1}/{EPOCHS} "
            f"Average Loss: {avg_loss:.6f} | Recon: {avg_recon_loss:.6f} | Diff: {avg_diff_loss:.6f} | Gene: {avg_gene_loss:.6f} "
