...
from src.models_v22_skip_connection import HyperNetwork_Perceiver_v22, GaborRenderer_v22, HyperNetwork_Perceiver_v23, GaborRenderer_v23
...
def inference_demo(hp):
    ...
    with torch.no_grad():
        ...
        # Consistency Model Multi-step Sampling
        num_steps = getattr(hp, "num_sampling_steps", 5)  # Default to 5 steps
        ...
        print(f"Starting {num_steps}-step consistency sampling...")
        for i in range(num_steps):
            ...
            c_skip = (sigma_data**2) / (t_expand**2 + sigma_data**2)
            c_out = (t_expand * sigma_data) / torch.sqrt(t_expand**2 + sigma_data**2)
            ...
            # 5. Render fields
            f_theta = cnf(z1_gen, coords_norm, c_in * x, t_target) # [1, T_CHUNK, N, C_OUT]
            
            # Apply rigorous skip connection integration
            x0_pred = c_skip * x + c_out * f_theta
