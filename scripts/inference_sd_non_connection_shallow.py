...
def inference_demo(hp):
    ...
    print("Loading ground truth sample from test dataset...")
    ...
    try:
        ...
        print(f"Ground truth sample loaded: shape {ground_truth_sample.shape}")
    except StopIteration:
        print("Warning: Could not load ground truth sample from dataset")
        ground_truth_sample = None
    ...
    if RENDERER_TYPE == "GaborRenderer_v22":
        ...
        print("Using GINO-based Renderer")
    ...
    with torch.no_grad():
        ...
        phi_uniq = np.unique(coord[:, 0])
        theta_uniq = np.unique(coord[:, 1])
        N_phi = len(phi_uniq)
        N_theta = len(theta_uniq)
        ...
        if field.shape[-1] > 1:
            all_values_pred = field[0, ..., 0]  # Only visualize vorticity (channel 0)
        else:
            all_values_pred = field[0].squeeze(-1)
        
        if field_gt is not None:
            if field_gt.shape[-1] > 1:
                all_values_gt = field_gt[0, ..., 0]
            else:
                all_values_gt = field_gt[0].squeeze(-1)
        ...
        if field_gt is not None:
            ...
            if data_gt_0.ndim == 2 and data_gt_0.shape[1] > 1:
                values_gt_0 = data_gt_0[:, 0]
            else:
                values_gt_0 = data_gt_0.squeeze()
            values_gt_0_2d = values_gt_0.reshape(N_phi, N_theta)

            data_pred_0 = field[0, 0]
            if data_pred_0.ndim == 2 and data_pred_0.shape[1] > 1:
                values_pred_0 = data_pred_0[:, 0]
            else:
                values_pred_0 = data_pred_0.squeeze()
            values_pred_0_2d = values_pred_0.reshape(N_phi, N_theta)
            ...
            im_gt = axes[0].imshow(values_gt_0_2d.T, cmap='RdBu_r', origin='lower', aspect='auto',
                                   extent=[0, 360, -90, 90], vmin=vmin, vmax=vmax)
            axes[0].set_title('Ground Truth at t=0')
            axes[0].set_xlabel('Longitude (phi)')
            axes[0].set_ylabel('Latitude (theta)')
            fig.colorbar(im_gt, ax=axes[0], label='Vorticity')

            im_pred = axes[1].imshow(values_pred_0_2d.T, cmap='RdBu_r', origin='lower', aspect='auto',
                                     extent=[0, 360, -90, 90], vmin=vmin, vmax=vmax)
            axes[1].set_title('Generated at t=0')
            axes[1].set_xlabel('Longitude (phi)')
            axes[1].set_ylabel('Latitude (theta)')
            fig.colorbar(im_pred, ax=axes[1], label='Vorticity')

            im_error = axes[2].imshow(error_0_2d.T, cmap='hot', origin='lower', aspect='auto',
                                      extent=[0, 360, -90, 90])
            axes[2].set_title('Absolute Error at t=0')
            axes[2].set_xlabel('Longitude (phi)')
            axes[2].set_ylabel('Latitude (theta)')
            fig.colorbar(im_error, ax=axes[2], label='|Error|')
            
            def update_comparison(t):
                # Ground Truth
                data_gt = field_gt[0, t]
                if data_gt.ndim == 2 and data_gt.shape[1] > 1:
                    values_gt = data_gt[:, 0]
                else:
                    values_gt = data_gt.squeeze()
                values_gt_2d = values_gt.reshape(N_phi, N_theta)

                # Prediction
                data_pred = field[0, t]
                if data_pred.ndim == 2 and data_pred.shape[1] > 1:
                    values_pred = data_pred[:, 0]
                else:
                    values_pred = data_pred.squeeze()
                values_pred_2d = values_pred.reshape(N_phi, N_theta)

                error_2d = np.abs(values_pred_2d - values_gt_2d)

                im_gt.set_data(values_gt_2d.T)
                axes[0].set_title(f'Ground Truth at t={t}')

                im_pred.set_data(values_pred_2d.T)
                axes[1].set_title(f'Generated at t={t}')
            ...
            def update(t):
                ...
                ax.set_title(f'Generated Field at time step {t}')
