...
class SinusoidalPositionEmbeddings(nn.Module):
    ...
    def forward(self, time):
        # time: [B]
        time = torch.clamp(time, min=1e-5) # Avoid extreme values that can cause instability in exp
        c_noise = 0.25 * torch.log(time) 
        half_dim = self.dim // 2
...
class GaborRenderer_v22(nn.Module):
    ...
    def __init__(self, latent_dim=256, coord_dim=2, t_chunk=16, channel_out=2, hidden_dim=256, num_layers=4, use_node_type=False):
        ...
        self.query_proj = nn.Sequential(
            nn.Linear(coord_dim + self.freq_dim * 2 * channel_out + self.node_type_num, hidden_dim),
            nn.GELU(),
            nn.Linear(hidden_dim, latent_dim)
        )
    ...
    def forward(self, z_multi, coords, x_t_out, node_type=None):
        B, N, _ = coords.shape
        ...
        # x_t_out [B, T, N, C] skip connection
        x_freq = torch.fft.rfft(x_t_out, dim=1)  # [B, F, N, C]
        x_freq_real = torch.view_as_real(x_freq).permute(0, 2, 1, 3, 4).reshape(B, N, -1)  # [B, N, F * C * 2]
        q = torch.cat([x0, x_freq_real], dim=-1) # [B, N, coord_dim + F * C * 2]
    
        if self.use_node_type and node_type is not None:
            node_type = node_type.squeeze(-1) # [B, N, 1] -> [B, N]
            node_type_emb = F.one_hot(node_type, num_classes=self.node_type_num).float() # [B, N, self.node_type_num]
            q = torch.cat([q, node_type_emb], dim=-1) # [B, N, coord_dim + self.node_type_num]
        q = self.query_proj(q)
        q = self.norm_q(q)
...
class GaborRenderer_v23(nn.Module):
    ...
    def __init__(self, latent_dim=256, coord_dim=2, t_chunk=16, channel_out=2, hidden_dim=256, num_layers=4, use_node_type=False):
        ...
        # x_t_out [B, T, N, C] skip connection projected to wavelet domain
        self.swt_levels = 3
        self.swt = TemporalSWT(levels=self.swt_levels)

        # Cross-attention to extract coordinate-specific latents
        self.query_proj = nn.Sequential(
            nn.Linear(coord_dim + (self.swt_levels + 1) * self.t_chunk * self.channel_out + self.node_type_num, hidden_dim),
            nn.GELU(),
            nn.Linear(hidden_dim, latent_dim)
        )
    ...
    def forward(self, z_multi, coords, x_t_out, node_type=None):
        ...
        # Extract location-specific latent via Cross-Attention
        x_swt_coeffs = self.swt(x_t_out) # List of 4 tensors of shape [B, N, C * T]
        x_swt_coeffs = torch.cat(x_swt_coeffs, dim=-1) # [B, N, (levels+1)*T*C]

        q = torch.cat([x0, x_swt_coeffs], dim=-1) # [B, N, coord_dim + (levels+1)*T*C]
        if self.use_node_type and node_type is not None:
            node_type = node_type.squeeze(-1) # [B, N, 1] -> [B, N]
            node_type_emb = F.one_hot(node_type, num_classes=self.node_type_num).float()
            q = torch.cat([q, node_type_emb], dim=-1)
        q = self.query_proj(q)
        q = self.norm_q(q)
        kv = self.norm_kv(z_multi)
        z, _ = self.cross_attn(q, kv, kv) # z is [B, N, latent_dim]
