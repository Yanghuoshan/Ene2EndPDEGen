...
from src.modules.diffusion import checkpoint
...
def Normalize(in_channels, type=TYPE):
    if type == "layer":
...
class CrossAttention(nn.Module):
    ...
    def forward(self, x, context=None, mask=None):
        ...
        if exists(mask): # assume mask is 1 for valid positions and 0 for invalid positions, and in shape b n
            mask = rearrange(mask, 'b ... -> b (...)')
            max_neg_value = -torch.finfo(sim.dtype).max
            mask = repeat(mask, 'b j -> (b h) () j', h=h)

            if not isinstance(mask, torch.BoolTensor):
                mask = mask > 0 # convert from 1s and 0s to boolean tensor
...
class SpatialTransformer(nn.Module):
    ...
    def forward(self, x, context=None):
        ...
        b, c, t, h, w = x.shape # x is in latent space (b, c, t, h, w)
        x_in = x
        x = self.norm(x)
        x = self.proj_in(x)
